...
class TwitterExportProcessor {
    ...
    async setConfUsername(username){await this.saveData("username", username);}
    async getConfUsername(){return await this.loadData("username")}
    ...
    async syncTweet(tweet){
        ...
        if(!_.isUndefined(tweetStored)){
            ...
            if(tweetStored.retweet_count < tweet.retweet_count){await this.updateTweet(tweet);}
            return;
        }
    ...
    async clearAllTweets(){await this.deleteData("tweets");}
    getStoredTweet(id_str){return this.loadData("tweets." + id_str);}
    getStoredTweets(){return this.loadData("tweets");}
    ...
    async getFriendshipStatus(user_ids){
        ...
        arrayChunks.forEach(function(chunk){
            ...
            chunkPromises.push(obj.promiseGet(obj.FriendshipStatusURL, params).then((values) => chunkResults.push(values)));
        });
