...
class TwitterGatewayAPI {
  ...
  async sendDM(args, dry_run) {
    if (_.isUndefined(dry_run)) {
      dry_run = true
    }
    if (_.isUndefined(args.recipient_id)) {
      return {id: '-2', err_msg: 'need recipient_id'}
    }
    ...
    if (dry_run) {
      ...
      return this.asyncFuncDelay({id: '-1', timestamp: '1593167166488'}, true).catch(e => {
        return e
      })
    } else {
      return await send_dm.dm(this.clientLite, args).catch(e => {
        return e
      })
    }
  }
  ...
  logAPICall(gateway_url) {
    ...
    gateway_url = gateway_url.includes(this.getRetweetsURL_SUBSTRING)
      ? this.getRetweetsURL_SUBSTRING
      : gateway_url
  ...
  getError(error) {
    if (_.isArray(error) && error.length > 0) {
      ...
      if (!_.isUndefined(errObj.message) && !_.isUndefined(errObj.code)) {
        return errObj
      }
    }
